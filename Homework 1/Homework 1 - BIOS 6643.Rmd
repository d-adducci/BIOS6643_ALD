...
Solved in one step by R (code found in appendix, pg.21), but one way steps can be broken down as follows:
...
Solved in one step by R (code found in appendix, pg.21). $\hat{Y}$ is found by multiplying X, the subjects covariate matrix, by $\hat{\beta}$, the vector of estimated coefficients. This returns the estimated outcomes for each subject, $\hat{Y}$. 
...
Solved in one step in R (code found in appendix, pg.21). The residual vector $\hat{E_i}$ is found by subtracting the actual outcomes by the predicted outcomes:
