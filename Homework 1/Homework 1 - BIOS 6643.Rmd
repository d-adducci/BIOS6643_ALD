...
cort_data <- read.csv("C:/Users/domin/Documents/Biostatistics Masters Program/Fall 2023/BIOS 6643 - Analysis of Longitudinal Data/Homework 1/cortdata.csv")
...
time_block_metrics <- function(data_file,group,time_block){
  # data_file: Refers to the data set.
  # group: refers to which case control group is desired (character string). 
  # time_point: Refers to the specific time blocks (character string).
  data_file_select <- data_file[data_file$casecontrol == group,]
  mean <- mean(data_file_select[,time_block])
  sd <- sd(data_file_select[,time_block])
  var <- var(data_file_select[,time_block])
  metrics <- cbind(mean,sd,var)
  colnames(metrics) <- c("mean","sd","var")
  return(metrics)
}

# Calculating mean, standard deviation, and variance for each group. 
time1_c <- time_block_metrics(cort_data,"c","Time1")
time1_p <- time_block_metrics(cort_data,"p","Time1")
time2_c <- time_block_metrics(cort_data,"c","Time2")
time2_p <- time_block_metrics(cort_data,"p","Time2")
time3_c <- time_block_metrics(cort_data,"c","Time3")
time3_p <- time_block_metrics(cort_data,"p","Time3")
time4_c <- time_block_metrics(cort_data,"c","Time4")
time4_p <- time_block_metrics(cort_data,"p","Time4")
