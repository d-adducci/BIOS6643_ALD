...
time_block_metrics <- function(data_file,time_block){
  ...
  mean <- round(mean(data_file[,time_block]), digits = 4)
  sd <- round(sd(data_file[,time_block]), digits = 4)
  var <- round(var(data_file[,time_block]), digits = 4)
...
# and by group. 
all_metrics <- as.matrix(sapply(time_blocks, function(x) time_block_metrics(cort_data,x)))
c_metrics <- as.matrix(sapply(time_blocks, function(x) time_block_metrics(cort_data_c,x)))
p_metrics <- as.matrix(sapply(time_blocks, function(x) time_block_metrics(cort_data_p,x)))
