...
nr_mle <- function(exp_data,theta_est,loglike,iterations){
  ...
  first_d <- D(loglike,"theta") # Taking first derivative of log likelihood
  second_d <- D(first_d,"theta") # Taking second derivative of log likelihood
  for(i in 1:iterations){
    ll_first_d <- sum(eval(first_d)) # summing log likelihood of first derivative
    ll_second_d <- sum(eval(second_d)) # Summing log likelihood of second derivative
...
theta_mle <- nr_mle(exp_data,0,ll_cauchy_form,10)
theta_mle
```
