...
There are more subjects in the clinical data than the variogram data. Will need to select subjects from the clinical data based on the variogram data. 
...
fev_fvc_race_bp <- ggplot(na.omit(clinical_data), aes(x = Race, y = POSTFEVFVC)) +
  ...
  labs(title = "FEV/FVC Boxplots by Race", x = "",y = "FEV/FVC") +
  theme_bw()
...
```
\
The above boxplots show the distribution of FEv/FVC for each race category. 
...
1. How should be handle the multiple codes for the VAS measurements?
